...
const fundingCollection = database.collection("funding");
    ...
    app.post("/funding", verifyToken, async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to create funding.", error: err.message });
      }
    });
    ...
    app.get("/funding", verifyToken, async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to fetch fundings", error: err.message });
      }
    });
    ...
    app.get("/users/search", async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to search donors.", error: err.message });
      }
    });
    ...
    app.patch("/users/:id/role", verifyToken, verifyAdmin, async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to update user role", error: err.message });
      }
    });
    ...
    app.patch(
      ...
      async (req, res) => {
        try {
          ...
          if (recipientDistrict !== undefined)
            updateFields.recipientDistrict = recipientDistrict;
          if (recipientUpazila !== undefined)
            updateFields.recipientUpazila = recipientUpazila;
      ...
      async (req, res) => {
        try {
          ...
          const fundingAgg = await fundingCollection
            ...
            .toArray();
          const totalFunding =
            fundingAgg.length > 0 ? fundingAgg[0].totalFunding : 0;
